...
app.post('/getDocs', async (req, res) => {
  ...
  console.log(text);
  try {
    const urlHash = createUrlHash(text);
    let index = "";
    if (urlHashToIndex[urlHash]) {
      console.log("URL already processed");
      index = urlHashToIndex[urlHash];
    } else {
      index = await documentationFinder(text, apiKey);
      urlHashToIndex[urlHash] = index;
    }
    res.status(200).json({ message: 'Documentation processed successfully', urlHash });
  } catch (error) {
    console.error('Error while getting documentation:', error);
